...
function parseCorrelationSources(sources, expectedCount = 1) {
  ...
  if (![0, 1, 2].includes(expectedCount)) throw correlationError('Unsupported correlation count', diagnostics);
  ...
  if (expectedCount !== 1) diagnostics.uiCorrelations = [...records.values()];
  ...
  if (records.size !== expectedCount) throw correlationError(`Expected exactly ${expectedCount === 0 ? 'zero' : expectedCount === 1 ? 'one' : expectedCount} unique UI correlation capture`, diagnostics);
