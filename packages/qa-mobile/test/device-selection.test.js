...
const launchPasswordDismissal = [{ visible: 'Save Password?', tap: 'Not Now', below: 'Save Password?' }];
assert.equal(validateDeviceDescriptors({ ...mobile, devices: { iosSimulator: { platform: 'ios', kind: 'simulator', idEnvKey: 'IOS_ID', appId: mobile.appId, launchUri, launchReadySelector: 'qa-environment-root', launchOverlayDismissIfVisible: launchPasswordDismissal } } }), true);
assert.throws(() => validateDeviceDescriptors({ ...mobile, devices: { androidEmulator: { ...mobile.devices.androidEmulator, launchOverlayDismissIfVisible: launchPasswordDismissal } } }), /only for iOS simulators/);
assert.throws(() => validateDeviceDescriptors({ ...mobile, devices: { iosSimulator: { platform: 'ios', kind: 'simulator', idEnvKey: 'IOS_ID', appId: mobile.appId, launchUri, launchReadySelector: 'qa-environment-root', launchOverlayDismissIfVisible: [{ visible: 'Save Password?', tap: '', below: 'Save Password?' }] } } }), /valid visible, tap, and optional below labels/);
