...
const iosMobile = { appId: mobile.appId, devices: { iosSimulator: { platform: 'ios', kind: 'simulator', idEnvKey: 'IOS_ID', appId: mobile.appId, runtimeTimeoutMultiplier: 1.5, manualPreparation: { acknowledgementEnvKey: 'IOS_AUTOFILL_DISABLED_ID', instructions: 'docs/ios-preparation.md' }, deterministicTextReset: { id: 'auth.login.qa-reset', readySelector: 'screen.auth.login', readyTimeoutMs: 30000 } } } };
const resolvedIos = resolveConfiguredDevice(iosMobile, 'iosSimulator', { IOS_ID: 'IOS-A', IOS_AUTOFILL_DISABLED_ID: 'IOS-A' }, { execute: executeIos });
assert.equal(resolvedIos.id, 'IOS-A');
assert.equal(resolvedIos.runtimeTimeoutMultiplier, 1.5);
assert.equal(resolvedIos.manualPreparation.acknowledgementEnvKey, 'IOS_AUTOFILL_DISABLED_ID');
assert.equal(resolvedIos.deterministicTextReset.id, 'auth.login.qa-reset');
assert.ok(iosCalls.every(([, args]) => !args.includes('IOS-B')));
assert.throws(() => resolveConfiguredDevice(iosMobile, 'iosSimulator', { IOS_ID: 'IOS-A' }, { execute: executeIos }), /IOS_AUTOFILL_DISABLED_ID=IOS-A/);
assert.throws(() => resolveConfiguredDevice(iosMobile, 'iosSimulator', { IOS_ID: 'IOS-A', IOS_AUTOFILL_DISABLED_ID: 'IOS-B' }, { execute: executeIos }), /IOS_AUTOFILL_DISABLED_ID=IOS-A/);
...
assert.equal(validateDeviceDescriptors({ ...mobile, devices: { iosSimulator: { platform: 'ios', kind: 'simulator', idEnvKey: 'IOS_ID', appId: mobile.appId, manualPreparation: { acknowledgementEnvKey: 'IOS_AUTOFILL_DISABLED_ID', instructions: 'docs/ios-preparation.md' } } } }), true);
assert.throws(() => validateDeviceDescriptors({ ...mobile, devices: { androidEmulator: { ...mobile.devices.androidEmulator, manualPreparation: { acknowledgementEnvKey: 'ANDROID_PREPARED_ID' } } } }), /only for iOS simulators/);
assert.throws(() => validateDeviceDescriptors({ ...mobile, devices: { iosSimulator: { platform: 'ios', kind: 'simulator', idEnvKey: 'IOS_ID', appId: mobile.appId, manualPreparation: { acknowledgementEnvKey: 'bad key' } } } }), /valid acknowledgementEnvKey/);
