...
const initialValue = { assertVisible: { id: field, text: '^2$' } };
...
const finalValue = { assertVisible: { id: field, text: '^3$' } };
...
assert.deepEqual(commands.slice(scrollIndex, scrollIndex + 7), [targetScroll, initialValue, initialFocus, focusPress, {
  extendedWaitUntil: { visible: { id: marker, text: '^focused$' }, timeout: 5000 },
}, { pressKey: 'backspace' }, input]);
assert.deepEqual(commands.slice(-2), [focused, finalValue]);
...
try {
  ...
  const generatedIndex = resumeCommands.findIndex((command) => command.scrollUntilVisible?.element?.id === field);
  assert.ok(generatedIndex >= 0);
  const generatedTail = resumeCommands.slice(generatedIndex);
